...
$(document).ready(function () {
    ...
    $("[href='#group-manage']").on('click', homeTemplate.groupManageInit);
});
...
function HomeTemplate() {
    ...
    this.groupManageInit = function () {
        new GroupManage().menuInit();
    }
}

function AccountInfo() {
    var ccButton = $('#CC-button');
    var ccrButton = $('#CCR-button');
    var csButton = $('#CS-button');
    var csrButton = $('#CCR-button');
    ...
    this.menuInit = function () {
        ...
        if (null != ccrButton) {
            ccrButton.on('click', this.createCompany)
        }
        if (null != csButton) {
            csButton.on('click', this.createStore)
        }
        if (null != csrButton) {
            csrButton.on('click', this.createStore)
        }
    };
...
function GroupManage() {
    ...
    var templateID = 'GM-template';
    var URL = '/store/sellers';
    var storeID=$("#GM-storeID").text();

    var gm = $('#group-manage');
    var table = gm.find('table');

    this.menuInit = function () {
        new SelectUtil().selectInitForJson(gm.find("[name='status']"), '/json/status_sys.json');
        this.tableInit();
        $("#GM-query").on('click', this.query);
    };
    ...
    this.tableInit = function () {
        $.getJSON(URL, {
            ...
            if (data.status) {
                table.html(template(templateID, data));
                var pages = data.data.pages;
                var pageNum = data.data.pageNum;
                var options = {
                    bootstrapMajorVersion: 3,//bootstrap版本
                ...
                $('#GM-page').bootstrapPaginator(options);
            } else {
                var modal = $.scojs_modal({
                    keyboard: true,
                    title: '错误提示',
    ...
    this.query = function () {
        $.getJSON(URL, {
            ...
            cityID: location.getCityVal(),
            areaID: location.getAreaVal(),
            status: gm.find("[name='status']").val(),
            storeID: storeID
        }, function (data) {
            if (data.status) {
                table.html(template(templateID, data));
